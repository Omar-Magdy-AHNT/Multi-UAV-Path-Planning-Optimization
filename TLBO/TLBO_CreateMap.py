from TLBO.TLBO_Const1 import *  # Import constants related to the simulation  # noqa: F403
...
def createmap():
    for k in range(numstudents):  # Iterate over each child  # noqa: F405
        for i in range(numdrones):  # Iterate over each drone  # noqa: F405
            ...
            Students[k].append(startpt)  # Add the start point to Droneinfo  # noqa: F405
            ...
            while added_points < numtrackp:  # noqa: F405
                ...
                if len(Students[k]) > (1 + ((numtrackp + 2) * i)) and not vertical_check(Students[k][-2],Students[k][-1], (x, y, z)):  # noqa: F405
                ...
                if  not Trackpointlinevalid(Students[k][-1], (x, y, z),Students[k],(len(Students[k])-1)):  # noqa: F405
                ...
                Students[k].append((x, y, z))  # Add the new track point to Droneinfo  # noqa: F405
                possible_points.remove((x, y, z))  # Remove the point from possible points
                added_points += 1  # Increment the count of successfully added track points
        
            Students[k].append(endpt)  # Append the endpoint after adding all track points  # noqa: F405